...
io.on("connection", (socket) => {
    ...
    try {
      ...
      if (insertedId) {
        const [result2] = await connection.execute(
          ...
          [data.message.Emergency, insertedId, data.message.Sex, data.message.Age, data.message.Description, 1,1]
        );
  ...
  socket.on("accept_message", async (data) => {
    ...
    if(resultaccept){
       const [resultemi]= await connection.execute("SELECT id_emi from petresemi where id_pac= ?",[data.folio])
       const [idEmisor]= await connection.execute("select ws_webid as websocketid from websocketid where id_wsid = ?",[resultemi[0].id_emi])
